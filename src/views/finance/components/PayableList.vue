    ...
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        ...
        <div v-if="showAll[index]" style="margin-top: 15px">
          <div style="margin-bottom: 15px"><strong>详细发票清单:</strong></div>
          <el-table
            :data="item.payableSheetContent"
            stripe
            style="width: 100%"
...
<script>
import { ApprovalPayable} from '../../../network/finance'
export default {
  name: "PayableList",
  props: {
    list: Array,
  ...
  methods: {
    ...
    approval(id) {
      ...
      ApprovalPayable(config).then(res => {
        this.$emit("refresh")
        this.$message({
          message: '操作成功!',
          type: 'success'
